...
impl Transition {
    ...
    pub fn eps_trigger(self, from: i32) ->bool{
...
pub fn  run ()->NFA{
    ...
    let test1 = Box::new(Exp::Char{val: 'b'});
    let test2 = Box::new(Exp::Conc{left: Box::new(Exp::Char{val: 'a'}) , right: Box::new(Exp::Char{val: 'b'})});
    let test3 = Box::new(Exp::Alt{left: Box::new(Exp::Char{val: 'a'}) , right: Box::new(Exp::Eps{})});
    let test4 = Box::new(Exp::Star{obj: Box::new(Exp::Char{val: 'c'})});
    let test5 = Box::new(Exp::Eps{});
    let test6 = Box::new(Exp::Phi{});
    let test7 = Box::new(Exp::Conc{left: Box::new(Exp::Conc{left: Box::new(Exp::Char{val: 'a'}) , right: Box::new(Exp::Char{val: 'b'})}) , right: Box::new(Exp::Conc{left: Box::new(Exp::Char{val: 'c'}) , right: Box::new(Exp::Char{val: 'd'})})});
    let test8 = Box::new(Exp::Star{obj: test3});
    //let test9 = Box::new(Exp::Conc{left: test8, right: test1});



    let a = TransformWorker{name_supply: 0};
    let b = a.transform(&test4);
