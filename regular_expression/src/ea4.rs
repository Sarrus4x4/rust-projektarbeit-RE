    ...
    fn transform_worker<S:RE>(self, re: S){
        ...
        match type_of(re.l){
            "regular_expression::re::Alt" => {
                type_of(re.l);
                n1 = self.transform_worker_alt(re.l); //<- this is my problem: no field `l` on type `S` (but if i use re as a explicid parameter type 'Alt<S,I> i can't make a nested call because its not generic)
            }
        }
        match type_of(re.r){
            "regular_expression::re::Alt" => {
                n2 = self.transform_worker_alt(re.r); //<- this is my problem: no field `l` on type `S` (but if i use re as a explicid parameter type 'Alt<S,I> i can't make a nested call because its not generic)
            }
        }
